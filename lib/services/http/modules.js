...
function modinfo(req, res, modname)
{
  getmodule(modname, function(module) {
    function dorender(err, files) {
      ...
      templates.render('module.html', {files: files, module: module, debug_plaintext: JSON.stringify(module, null, 4)}, req, res);
    }
    if (module.meta && module.meta.source_files_path) {
      var p =path.join(module.meta.source_files_path, module.name);
      console.log('Path to module: '+ p);
      fs.get_path_file_list_array(p, null, dorender);
    }
    else {
