...
SourceDownloader.prototype.process_pending_packages = function(interval) {
  ...
  db.collection.find({'meta.tarball_path': null, 'meta.source_files_path': null}, {'limit': PACKAGES_AT_ONCE}, function(err, cursor) {
      ...
      async.forEachSeries(results, function(doc, callback) {
        ...
        self.download_tarball(doc.tarball_url, function(err, tarball_path) {
          ...
          self.extract_tarball(tarball_path, extract_path, function(err) {
            ...
            var query = { 'meta.tarball_path': tarball_path,
                          'meta.source_files_path': extract_path,
                          'meta.updated': new Date()
            }
            
            db.collection.update({'_id': package_id}, { '$set': query }, function(err, doc) {
        ...
        response.emit('close');
          ...
          if (!errstate) {
            return callback(null, file_path);
				...
				log.err(exx+": File stream not writeable, unable to store file ("+file_path+")");
			}
    });
  });
};
...
SourceDownloader.prototype.extract_tarball = function(tarball_path, extract_path, cb) {
    ...
    function(err) {
      ...
      var command = sprintf('%s -xzf %s -C %s --strip-components 1', (config.get().use_gtar ? 'gtar' : 'tar'), tarball_path, extract_path);
