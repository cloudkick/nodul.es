  ...
  async.forEachSeries(names, function(name, callback) {
    ...
    async.parallel([
      ...
      function(callback) {
        var pkg = root[name];
        var repo;
        if (pkg.repository && pkg.repository.url && pkg.repository.url.match(/^(http|git)\:\/\/github\.com/)) {
          // Figure out the GH url
          try {
            repo = pkg.repository.url.match(/^(http|git)\:\/\/github\.com\/(.+)\.git$/)[2];
          }
          catch (err) {
            log.err("Error matching github URL (" + pkg.repository.url + "): " + err);
            return callback();
          }
...
PackageIndexer.prototype.index_package = function(pkg, cb) {
  ...
  setTimeout(function() {
    log.debug("Indexed package: " + pkg.basics.name);
    log.debug(JSON.stringify(pkg));
    ps.emit('indexer.indexing.package', pkg.basics.name);
    
    package_data = db.format_package_data_for_storage(pkg);
    db.collection.insert([package_data], function(docs) {
