      ...
      function(callback) {
        ...
        if (pkg.basics.repository && pkg.basics.repository.url && pkg.basics.repository.url.match(/^(http|git)\:\/\/github\.com/)) {
          ...
          try {
            ...
            try {
              repo = pkg.basics.repository.url.match(/^(http|git)\:\/\/github\.com\/(.+)$/)[2];
            }
            catch (err) {
              log.err("Error matching github URL (" + pkg.basics.repository.url + "): " + err);
              return callback();
          ...
          github_get('/commits/show/' + repo + '/master', function(response, commits) {
            if (response.statusCode !== 200) {
              ...
              try {
                pkg.github = {commit: commits.commit};
              }
              catch (err) {
                pkg.github = undefined;
              }
              callback();
            }
          });
...
PackageIndexer.prototype.index_package = function(pkg, cb) {
  ...
  function (callback) {
    if (!package_data.latest_commit || !package_data.latest_commit.id) {
      log.debug(sprintf('No available commits for the package %s, skipping updating latest commits', package_data.name));
      return callback();
    }
    ...
    db.collection.findOne(query, function(err, doc) {
      ...
      if (doc.latest_commit.id != package_data.latest_commit.id) {
        doc_updated = doc;
        doc_updated.latest_commit = package_data.latest_commit;

        // New commits, update the package commit data
        db.collection.update({'_id': package_data._id}, doc_updated, function(err, doc) {
          if (err) {
            log.err("Error updating package data: " + err.message);
          }
          else {
            log.debug(sprintf('Updated latest commits for package %s, version: %s', package_data.name, package_data.latest_version));
            ps.emit('indexer.updated_commits.package', package_data.name);
          }
